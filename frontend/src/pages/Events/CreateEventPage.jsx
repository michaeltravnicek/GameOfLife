...
export default function CreateEventPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(err.response?.data?.error || 'Chyba při vytváření akce.');
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="ev-card">
            <div className="ev-grid-2">
              <div className="ev-field">
                ...
                <input className="ev-input" id="f-lat" type="number" step="0.0001" value={form.latitude} onChange={setField('latitude')} placeholder="49.1951" />
              </div>
              <div className="ev-field">
                <label htmlFor="f-long">Zeměpisná délka (long)</label>
                <input className="ev-input" id="f-long" type="number" step="0.0001" value={form.longitude} onChange={setField('longitude')} placeholder="16.6068" />
              </div>
            </div>
          </div>
        </section>
